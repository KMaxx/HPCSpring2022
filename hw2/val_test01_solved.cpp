...
//
{
  ...
  x = ( int * ) malloc ( n * sizeof ( x ) );	// changed int -> x in sizeof() because it was taking the size of the worng thing
  ...
  free(x);	// changed delete[] -> free() because delete was used incorrectly instead of free
